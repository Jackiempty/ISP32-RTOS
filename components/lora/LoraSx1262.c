...
static lora_t lora_instance = {.rssi = 0, .snr = 0, .signalRssi = 0};

static bool inReceiveMode = false;
static uint8_t spiBuff[32];  // Buffer for sending SPI commands to radio

// Config variables (set to PRESET_DEFAULT on init)
static uint32_t pllFrequency;
static uint8_t bandwidth;
static uint8_t codingRate;
static uint8_t spreadingFactor;
static uint8_t lowDataRateOptimize;
static uint32_t transmitTimeout;  // Worst-case transmit time depends on some factors
...
bool lora_init() {
  ...
  gpio_reset_pin(SX1262_NSS);
  gpio_set_direction(SX1262_NSS, GPIO_MODE_OUTPUT);
  gpio_set_level(SX1262_NSS, 1);  // High = inactive

  gpio_set_direction(SX1262_RESET, GPIO_MODE_OUTPUT);
  gpio_set_direction(SX1262_DIO1, GPIO_MODE_INPUT);  // Radio interrupt pin.  Goes high when we receive a packet

  // Hardware reset the radio by toggling the reset pin
  gpio_set_level(SX1262_RESET, 0);
  delay(100);
  gpio_set_level(SX1262_RESET, 1);
  delay(100);
  gpio_set_level(SX1262_RESET, 0);
  delay(100);
  gpio_set_level(SX1262_RESET, 1);
 ...
 */
bool sanityCheck() {
  uint16_t addressToRead[2] = {0x07, 0x40};

  delay(10);

  gpio_set_level(SX1262_NSS, 0);  // CS Low = Enabled
  spi_transfer(0x1D);             // OpCode for "read register"
  spi_transfer(addressToRead[0]);
  spi_transfer(addressToRead[1]);
  spi_transfer(0x00);                     // Dummy byte
  uint8_t regValue = spi_transfer(0x00);  // Read response
  gpio_set_level(SX1262_NSS, 1);          // CS High = Disabled

  ESP_LOGI(TAG, "%x", regValue);
...
void configureRadioEssentials() {
  ...
  gpio_set_level(SX1262_NSS, 0);  // Enable radio chip-select
  spiBuff[0] = 0x9D;              // Opcode for "SetDIO2AsRfSwitchCtrl"
  spiBuff[1] = 0x01;              // Enable
  spi_transfer(spiBuff[0]);
  spi_transfer(spiBuff[1]);
  gpio_set_level(SX1262_NSS, 1);  // Disable radio chip-select
  ...
  spiBuff[1] = 0x00;              // Stop timer on:  0x00=SyncWord or header detection, 0x01=preamble detection  spi_transfer(spiBuff,2);
  gpio_set_level(SX1262_NSS, 1);  // Disable radio chip-select
  delay(100);                     // Give time for radio to process the command
...
void transmit(byte *data, int dataLen) {
  ...
  spi_transfer(spiBuff[0]);       // Send header info
  spi_transfer(spiBuff[1]);

  // spi_transfer overwrites original buffer.  This could probably be confusing to the user
  ...
  for (uint16_t i = 0; i < dataLen; i += size) {
    ...
    for (int j = 0; j < size; j++) {
      spi_transfer(spiBuff[j]);  // Write the payload itself
    }
  }
...
bool waitForRadioCommandCompletion(uint32_t timeout) {
  uint32_t startTime = bsp_current_time();
  ...
  while (!dataTransmitted) {
    ...
    if (bsp_current_time() - startTime >= timeout) {
      return false;
    }
  }
...
A max length of the buffer can be provided to avoid buffer overflow.  If buffer is not large enough for entire payload, overflow is thrown
out. Recommended to pass in a buffer that is 255 bytes long to make sure you can received any lora packet that comes in.

Returns -1 when no packet is available.
Returns 0 when an empty packet is received (packet with no payload)
Returns payload size (1-255) when a packet with a non-zero payload is received. If packet received is larger than the buffer provided, this
will return buffMaxLen
*/
int lora_receive_async(byte *buff, int buffMaxLen) {
  ...
  lora_instance.rssi = -((int)spiBuff[2]) / 2;   //"Average over last packet received of RSSI. Actual signal power is –RssiPkt/2 (dBm)"
  lora_instance.snr = ((int8_t)spiBuff[3]) / 4;  // SNR is returned as a SIGNED byte, so we need to do some conversion first
  lora_instance.signalRssi = -((int)spiBuff[4]) / 2;
  ...
  for (int i = 0; i < 3; i++) {
    spi_transfer(spiBuff[i]);  // Send commands to get read started
  }
  for (int i = 0; i < payloadLen; i++) {
    spi_transfer(buff[i]);  // Get the contents from the radio and store it into the user provided buffer
  }
  gpio_set_level(SX1262_NSS, 1);  // Disable radio chip-select
...
This will store the contents of the payload into the user-provided buffer.  Recommended to use a buffer with 255 bytes to always receive
full packet. If a smaller buffer is used, maxBuffLen can be set to avoid buffer overflow. Any packets larger than this will have remaining
bytes thrown out.
...
will return buffMaxLen
*/
int lora_receive_blocking(byte *buff, int buffMaxLen, uint32_t timeout) {
  setModeReceive();  // Sets the mode to receive (if not already in receive mode)
  ...
  while (gpio_get_level(SX1262_DIO1) == false) {
    // If user specified a timeout, check if we hit it
    if (timeout > 0) {
      elapsed = bsp_current_time() - startTime;
      if (elapsed >= timeout) {
...
// You must set pllFrequency before calling this
...
void updateModulationParameters() {
  ...
  spiBuff[3] = codingRate;  // ModParam3 = CodingRate.  Semtech recommends CR_4_5 (which is 0x01).  Options are 0x01-0x04, which correspond
                            // to coding rate 5-8 respectively
...
bool configSetBandwidth(int bandwidth) {
  ...
  if (bandwidth < 0 || bandwidth > 0x0A || bandwidth == 7) {
    return false;
  }
  bandwidth = bandwidth;
  updateModulationParameters();
...
bool configSetSpreadingFactor(int spreadingFactor) {
  ...
  lowDataRateOptimize = (spreadingFactor >= 11) ? 1 : 0;  // Turn on for SF11+SF12, turn off for anything else
