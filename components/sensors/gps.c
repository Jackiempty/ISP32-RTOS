...
static inline void gps_parser(uint8_t *raw) {
    ...
    if (strcmp((char *)&parsed.field[0][3], "GGA") == 0) {
        ...
        if (parsed.field[4][0] != 0) {
            gps_instance.longitude = (CHAR2INT(parsed.field[4][0]) * 100 + CHAR2INT(parsed.field[4][1]) * 10 +
                                      CHAR2INT(parsed.field[4][2])) *
                                         600000 +
                                     CHAR2INT(parsed.field[4][3]) * 100000 + CHAR2INT(parsed.field[4][4]) * 10000 +
                                     CHAR2INT(parsed.field[4][6]) * 1000 + CHAR2INT(parsed.field[4][7]) * 100 +
                                     CHAR2INT(parsed.field[4][8]) * 10 + CHAR2INT(parsed.field[4][9]);
...
gps_t *gps_fetch() { return &gps_instance; }
